(ns singpass.main
  "Kotodama WASM entrypoint for the Singpass clean-room actor (L4) — Clojure port.
  ...
  facts (`singpass.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "singpass")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"singpass.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "singpass-compat" :tier tier :entities entities} 200])
...
(defn start! [] :singpass-compat/ready)
